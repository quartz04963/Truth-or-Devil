...
public class Tutorial : MonoBehaviour
{
    ...
    public void RevisedInit()
    {
        ...
        // 정렬 기준 "천사/악마" 비활성화
        if (GameManager.Instance.currentStage <= StageDataList.Chapter0 + StageDataList.Chapter1)
        {
    ...
    public void HighlightTiles(List<int> redBoxData, List<int> blueBoxData)
    {
        // if (GameManager.Instance.CurrentStage == 1)
        // {
        //     MapManager.instance.objects.ForEach(obj => obj.HighlightTile(false));
        //     MapManager.instance.gates.ForEach(gate => gate.HighlightArea(false));

        //     if ((RedData)redBoxData[0] == RedData.Gate && (BlueData)blueBoxData[0] == BlueData.Null)
        //     {
        //         MapManager.instance.gates.ForEach(gate => gate.HighlightArea(true));
        //     }
        ...
        //         {
        //             if (tile.color == (TileColor)blueBoxData[1]) MapManager.instance.objects.Find(obj => obj.pos == tile.pos).HighlightTile(true);
        //         }
        //     }

        //     else if ((RedData)redBoxData[0] == RedData.Gate && (BlueData)blueBoxData[0] == BlueData.Color)
        //     {
        //         MapManager.instance.gates.ForEach(gate => gate.HighlightArea(true, false));

        //         foreach (TDGate gate in MapManager.instance.gates)
        //         {
        //             foreach (TileData tile in MapManager.instance.tiles)
        //             {
        //                 if (Math.Abs(tile.pos.x - gate.pos.x) <= 1 && Math.Abs(tile.pos.y - gate.pos.y) <= 1 && tile.pos != gate.pos 
        //                     && tile.color == (TileColor)blueBoxData[1])
        //                     MapManager.instance.objects.Find(obj => obj.pos == tile.pos).HighlightTile(true);
        //             }
        //         }
        //     }
        // }
        ...
        //     {
        //         MapManager.instance.objects.ForEach(obj => obj.HighlightTile(true, false));
        //     }
