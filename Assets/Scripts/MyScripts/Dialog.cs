...
public static class TDStory
{
    ...
    static TDStory()
    {
        dialogList.Add(new TDDialog(1, true, new List<TDLine>
        {
            ...
            new TDLine("비델", "상자 3개를 모두 채우면, 그 내용들의 조합이 바로 참가자 분이 구성하신 질문입니다. <color=#FF0000>천국 문의 1칸 주변</color>에, <color=#0000FF>특정 색깔</color>이, <color=#109010>특정 개수</color>만큼 있냐, 이 뜻이죠."),
            new TDLine("비델", "지금은 <color=#FF0000>천국 문의 1칸 주변</color>에 <color=#0000FF>적색 칸</color>이 <color=#109010>딱 1개</color>가 있냐, 이런 질문을 하실 수 있는 겁니다. 참고로 ‘1칸 주변’이라는 건 대각선 포함이고요!"),
        ...
        dialogList.Add(new TDDialog(2, true, new List<TDLine> 
        {
            ...
            new TDLine("나겔", "첫째, <color=#FF00FF>질문 상자에 등록되는 내용은 덮어 씌워집니다.</color> 내용을 하나씩만 등록할 수 있으니, 같은 색의 다른 칸을 밟으시면 마지막에 밟은 칸의 내용만 남습니다."),
        ...
        dialogList.Add(new TDDialog(5, true, new List<TDLine>
        {
            ...
            new TDLine("", "이제 왼쪽 위의 시나리오 기능을 사용하실 수 있습니다."),
            new TDLine("", "시나리오 기능은 각 아이콘을 클릭해 경우의 수를 기록할 수 있는 기능으로, 실제 게임에 영향을 미치지는 않습니다."),
            new TDLine("", "여러 개의 시나리오를 추가할 수 있으며, 특정 시나리오를 클릭 후 ‘적용하기’ 버튼을 눌러 현재 스테이지에 해당 시나리오를 적용할 수 있습니다."),
        }));
        ...
        dialogList.Add(new TDDialog(8, true, new List<TDLine>
        {
            ...
            new TDLine("비델", "<color=#FF0000>이 무대 전체</color>에, <color=#0000FF>천사 혹은 악마</color>가, <color=#109010>총 몇 분</color>이 있냐, 이 질문에 대해 답을 해주시는 거죠."),
        ...
        {
            new TDLine("비델", "직원 분이 1분밖에 없는데 어떻게 악마가 2명이겠어요. 당연히 저 분이 악마입니다."),
            new TDLine("비델", "뭐, 생각보다 쉽죠? MAP이니 뭐니 해도, 정말 간단한 규칙이니까 앞으로도 안심하고 풀어주시면 됩니다!"),
            new TDLine("나겔", "1막 끝까지 성공하신 분 별로 없지 않아요?"),
            new TDLine("비델", "말이 그렇다는 거죠."),
        }));

        dialogList.Add(new TDDialog(9, true, new List<TDLine>
        {
            new TDLine("비델", "언제나 고생하시는 우리 직원 분들 보니까, 저도 참 옛날 기억이 새록새록 하네요."),
            new TDLine("나겔", "비델 님도 입사하고 보조 출연부터 시작하셨어요?"),
            new TDLine("비델", "제가 말한 기억은... 우리 신입 분이 무대에서 뛸 때 답변 잘못했다고 얼차려 당한 일이었는데요? 그게 100년 전이었나?"),
            new TDLine("나겔", "...89년 전이요."),
            new TDLine("비델", "뭐 어때요, 지금 진행자면 됐지. 이번에도 참가자님 힌트나 하나 주세요."),
            new TDLine("나겔", "사실 이번 무대는 크게 힌트라 할 건 없는데요."),
            new TDLine("나겔", "굳이 꼽자면, 생각보다 쉬우니까 너무 어렵게 생각하지 맙시다?"),
        }));
        ...
        dialogList.Add(new TDDialog(11, false, new List<TDLine>
        {
            ...
            new TDLine("비델", "이전에 알려드린 기법을 통해 C 직원이 악마라는 점, 그러니 경우의 수 중 하나가 사라진다는 점은 비교적 쉽게 알 수 있었습니다."),
            new TDLine("나겔", "그리고 <color=#FF0000>GATE</color> <color=#0000FF>GREEN</color> <color=#109010>1</color>이란 질문을 B 직원과 C 직원에게 동시에 해서, 둘이 답변이 달랐다는 점까지 알아내야 됐죠."),
            new TDLine("비델", "앞서 말씀드렸듯이 꽤 어려운 퍼즐이었습니다. 여기서 종종...음? 여보세요?"),
        }));
        ...
        dialogList.Add(new TDDialog(13, true, new List<TDLine>
        {
        ...
        dialogList.Add(new TDDialog(13, false, new List<TDLine>
        {
